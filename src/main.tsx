...
import {McciaLogo} from './mccia-logo';
import './styles.css';
...
function App(){
 ...
 if(me.isPending)return (
  <div className="loading-screen">
   <McciaLogo height={48}/>
   <h2>Review Desk</h2>
   <p>Loading your controlled change workspace…</p>
  </div>
 );
 if(me.isError&&(!(me.error instanceof ApiError)||me.error.status!==401))return (
  <div className="loading-screen" role="alert">
   <IconAlertTriangle size={42} color="#DC2626"/>
   <h2>Workspace service unavailable</h2>
   <p>We could not reach your workspace service. Free hosting may need a minute to wake.</p>
   <button className="button primary" onClick={()=>me.refetch()}>Retry connection</button>
  </div>
 );
