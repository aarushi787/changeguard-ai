...
`render.yaml` now defines a single **free pilot** Docker service in Singapore, without a paid disk. Startup uses `python -m scripts.start_cloud`: migrations must succeed before the server listens on Render's PORT. `STORAGE_BACKEND=database` stores document/report bytes in the private Supabase `document_blobs` table, so filesystem resets do not erase evidence. Keep one service instance with its embedded worker. Free Render instances sleep after idle periods, may take about a minute to wake, and are subject to workspace hours, build, bandwidth and outbound-traffic limits. This is a demonstration deployment, not an always-on production service. Do not enable paid options without approval.

Set DATABASE_URL through Render's secret environment settings. APP_ORIGIN is the Vercel frontend URL, ENVIRONMENT=production, RUN_WORKER=1, STORAGE_BACKEND=database and ALLOW_REGISTRATION=0. Existing demo accounts can sign in; company registration remains intentionally closed unless explicitly enabled. For this existing fictional Supabase demo, RESTORE_BUNDLED_DEMO=1 restores only missing bundled CSV files matching the saved source hash and never overwrites differing evidence. It does not migrate real customer documents. A conservative default 200 MB aggregate document budget and 25 MB individual blob limit constrain this pilot; ordinary upload limits still apply. Orphan blobs are retained rather than silently deleted. Larger workloads need dedicated object storage and reviewed retention.

Before enabling the cloud worker against this database, stop any local Supabase worker still using filesystem storage. All processes that share a queue must use the same storage backend. Transfer any non-demo documents through an authenticated administrative channel and verify hashes before switching traffic. Keep the old SQLite installation separate. Database-backed reads verify SHA-256, keys cannot be overwritten, and the API authorizes tenant-scoped references before retrieving bytes. No public object-download API or Supabase Data API grant is introduced.
