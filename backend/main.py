...
app=FastAPI(title='Review Desk',version='0.1.0',lifespan=lifespan)
...
def dependency_template(kind:Literal['process','control-plan','pfmea'],u:User=Depends(user)):
    ...
    return Response(data,media_type='application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',headers={'Content-Disposition':f'attachment; filename="ReviewDesk-{kind}-template.xlsx"'})
...
def report_download(id:str,u:User=Depends(user),s:DBSession=Depends(db)):
    ...
    return Response(content,media_type='application/pdf' if fmt=='pdf' else 'application/json' if fmt=='json' else 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet',headers={'Content-Disposition':f'attachment; filename="ReviewDesk-{id[:8]}.{fmt}"'})
